...
export class NewSidebarComponent implements OnInit {
  ...
  public timeLineData:timelinetabs[]=[{"name":"What to Expect","status":"pending","id":"1","link":"sim-getting-started13.0"},{"name":"","status":"initial","id":"2","link":"ftu-init-payments13.1"},{"name":"Run your first simulation","status":"initial","id":"3","link":"ftu-init-payments9.0"},{"name":"Go to the Dashboard","status":"initial","id":"4","link":""}]
  ...
  ngOnInit(): void {
    ...
    if(currentUrl=='sim-getting-started13.0' || currentUrl=='/sim-getting-started13.0'){
      this.active="What to Expect";  
    this.timeLineData[0].status='initial';
    this.timeLineData[1].status='initial';
    this.timeLineData[2].status='initial';
    }
    else if(currentUrl.split('?')[0]=='/ftu-init-payments13.1'){
    ...
    else if(currentUrl.split('?')[0]=='/ftu-init-payments9.0'){
  ...
  activeTab(event) {
    ...
    if(event.link=='ftu-init-payments13.1'){
      this.router.navigate(['ftu-init-payments13.1'], { queryParams: {selectedAddon : sessionStorage.getItem('selectedAddOn') } });
  ...
  next(){
    if(this.active=='What to Expect'){
      ...
      this.timeLineData[0].status='initial';
      this.timeLineData[2].status='pending';
      this.router.navigate(['ftu-init-payments9.0']);
    }
  ...
  exit(){
    if(this.active=='What to Expect'){
      ...
      this.timeLineData[0].status='initial';
      this.timeLineData[1].status='pending';
      this.router.navigate(['sim-getting-started13.0'], { queryParams: {selectedAddon : sessionStorage.getItem('selectedAddOn') } });
    }
    else if(this.active=="View Payment Rails"){
      this.active="What to Expect";
      this.timeLineData[0].status='initial';
